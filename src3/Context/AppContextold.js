...
export const AppProvider = ({ children, navigationState }) => {
  ...
  const shouldShowFooter = useCallback(() => {
    if (!navState || !navState.routes || navState.index === undefined) return false;
    
    try {
      const currentRoute = navState.routes[navState.index];
      const routeName = currentRoute?.name;
      
      // Hide footer on these screens
      const hideOnScreens = [
        'ProviderOTPLogin',
        'Login',
        'Splash',
        ...
        'KYCStatus',
        'Support',
        // Add other screens where you don't want footer
      ];
 ...
 console.log(navigationState)
  ...
  return (
    <AppContext.Provider value={contextValue}>
      ...
      {shouldShowFooter() && !loadingStates.footer && (
        <FooterMenu state={navState} />
      )}
